...
function hst_Logger_AddEntry( $type, $event, $message )
{

	//reading enabled log setting
	$optionLogErrors = get_option( hst_consts_optionKeyEventLogLogErrors, 1 );
	$optionLogEmails = get_option( hst_consts_optionKeyEventLogLogEmails, 0 );
	$optionLogDebug = get_option( hst_consts_optionKeyEventLogLogDebug, 0 );
	...
	if ( $writeThisLog == 1 )
	{
		...
		if ( hst_Logger_CheckFileWritable() )
		{

			// Logging class initialization
			$log = new hst_Logger_helper();
...
function hst_Logger_CheckFileWritable()
{

	$result = false;

	if ( is_readable( hst_consts_logFilePath ) )
	{

		if ( is_writeable( hst_consts_logFilePath ) )
		{
...
//Clears the log file
function hst_Logger_ClearLogFile()
{

	//Checks that the file exists and clears it
	if( file_exists( hst_consts_logFilePath ) )
	{

		if ( hst_Logger_CheckFileWritable() )
		{

			file_put_contents( hst_consts_logFilePath , "");
...
function hst_Logger_ReadLogFile()
{
	...
	if( file_exists( hst_consts_logFilePath ) )
	{
		...
		if ( hst_Logger_CheckFileWritable() )
		{
			...
			$arrayLines = ( file( hst_consts_logFilePath , FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES));
...
function hst_Logger_ReturnSize()
{

	if ( hst_Logger_CheckFileWritable() == true )
	{
		return filesize(hst_consts_logFilePath);
	}
	else
	{
		return -1;
...
function hst_Logger_TimeToClearIt()
{

	// 10000000 = 10megabytes roughly

	if ( hst_Logger_ReturnSize() > 10000000 )
	{
		return true;
	}
	else
...
function hst_Logger_exportVarContent( $variable )
{
...
class hst_Logger_Helper {
	...
	private function lopen() {
		...
		try
		{
			...
			$this->fp = fopen( hst_consts_logFilePath, 'a') or exit();
